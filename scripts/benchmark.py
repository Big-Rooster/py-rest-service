...
class RestBenchmark:
    ...
    async def create_example_request(self, index: int) -> tuple[float, bool]:
        ...
        try:
            payload = {
                "name": f"benchmark-py-rest-{index}",
                "description": f"Benchmark test py-rest {index}"
            }
            
            response = await self.client.post("/api/v1/py_rests", json=payload)
    ...
    async def get_example_request(self, example_id: str) -> tuple[float, bool]:
        ...
        try:
            response = await self.client.get(f"/api/v1/py_rests/{example_id}")
            end_time = time.perf_counter()
    ...
    async def list_examples_request(self) -> tuple[float, bool]:
        ...
        try:
            ...
            response = await self.client.get("/api/v1/py_rests", params=params)
            end_time = time.perf_counter()
