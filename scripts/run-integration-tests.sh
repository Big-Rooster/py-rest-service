...
cleanup() {
    if [[ "$CLEANUP" == "true" ]]; then
        ...
        docker volume rm $(docker volume ls -q | grep "py-rest-service") 2>/dev/null || true
        # Remove only this project's network
        docker network rm py-rest-service-network 2>/dev/null || true
    fi
}
...
build_packages() {
    ...
    for package in py-rest-service-persistence py-rest-service-api py-rest-service-core py-rest-service-client py-rest-service-server py-rest-service-integration-tests; do
...
start_docker_stack() {
    ...
    for i in {1..36}; do  # 36 attempts = 3 minutes max
        ...
        if [ $i -eq 36 ]; then
            ...
            docker logs py-rest-service 2>&1 | tail -30
            exit 1
        fi
...
verify_services() {
    ...
    if ! curl -s -f http://localhost:8080/health >/dev/null 2>&1; then
        ...
        print_status "Service logs:"
        docker logs py-rest-service 2>&1 | tail -20
        return 1
    fi
    
    # Quick database check
    if ! docker-compose exec -T postgres pg_isready -U postgres -d py_rest_service 2>/dev/null; then
...
run_tests() {
    ...
    cd "$PROJECT_ROOT/py-rest-service-integration-tests"
    
    # Set environment variables
    export API_SERVER_HOST=localhost
    export API_SERVER_PORT=8000
    export MANAGEMENT_PORT=8080
    export DATABASE_URL="postgresql://postgres:postgres@localhost:5432/py_rest_service"
