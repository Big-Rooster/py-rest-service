"""REST HTTP client for PyRest Service."""
...
from ybor_technologies.big_rooster.py_rest.service.api.models import (
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    raise PyRestServiceClientError(f"Token refresh failed: {response.status_code}")
        finally:
            self._refresh_lock = False
...
class PyRestServiceClientError(Exception):
    """Base exception for PyRest service client errors."""
...
class PyRestServiceClient:
    """Client for connecting to the PyRest Service REST API."""
    ...
    def __init__(
        ...
        """Initialize the PyRest Service client.
        ...
        logger.info(
            "PyRest Service client initialized",
            base_url=base_url,
            timeout=timeout,
    ...
    @classmethod
    def create(cls, base_url: str, timeout: float = 30.0) -> "PyRestServiceClient":
    ...
    async def _make_authenticated_request(
        ...
        **kwargs
        ...
        Raises:
            PyRestServiceClientError: If the request fails
        """
    ...
    async def login(self, username: str, password: str) -> Dict[str, Any]:
        ...
        Raises:
            PyRestServiceClientError: If login fails
        """
        try:
            ...
            raise PyRestServiceClientError(f"Network error during login: {str(e)}")
        except Exception as e:
            logger.error("Unexpected error during login", error=str(e), exc_info=True)
            raise PyRestServiceClientError(f"Unexpected error during login: {str(e)}")

    async def create_py_rest(self, py_rest: PyRestDto) -> CreatePyRestResponse:
        """Create a new py_rest.
        
        Args:
            py_rest: PyRest data to create
            
        Returns:
            Response containing the created py_rest
            
        Raises:
            PyRestServiceClientError: If the API call fails
        """
        logger.info("Creating py_rest", name=py_rest.name)
        
        try:
            # Convert DTO to JSON payload
            payload = py_rest.model_dump(exclude_none=True)
            
            # Make authenticated REST API call
            response = await self._make_authenticated_request(
                "POST",
                f"{self.base_url}/api/v1/py_rests",
                json=payload
            )
            ...
            if response.status_code == 201:
                ...
                result = CreatePyRestResponse(**response_data)
                logger.info("PyRest created successfully", py_rest_id=result.py_rest.id)
                return result
            else:
                await self._handle_error_response(response, "creating py_rest")
                
        except httpx.RequestError as e:
            logger.error("Network error creating py_rest", error=str(e))
            raise PyRestServiceClientError(f"Network error: {str(e)}")
        except Exception as e:
            logger.error("Unexpected error creating py_rest", error=str(e), exc_info=True)
            raise PyRestServiceClientError(f"Unexpected error: {str(e)}")

    async def get_py_rests(self, request: GetPyRestsRequest) -> GetPyRestsResponse:
        """Get a paginated list of py_rests.
        
        Args:
            request: Pagination request parameters
            
        Returns:
            Response containing py_rests and pagination metadata
            
        Raises:
            PyRestServiceClientError: If the API call fails
        """
        logger.info("Getting py_rests", start_page=request.start_page, page_size=request.page_size)
        ...
        try:
            ...
            if response.status_code == 200:
                response_data = response.json()
                result = GetPyRestsResponse(**response_data)
                logger.info("PyRests retrieved successfully", count=len(result.py_rests))
                return result
            else:
                await self._handle_error_response(response, "getting py_rests")
                
        except httpx.RequestError as e:
            logger.error("Network error getting py_rests", error=str(e))
            raise PyRestServiceClientError(f"Network error: {str(e)}")
        except Exception as e:
            logger.error("Unexpected error getting py_rests", error=str(e), exc_info=True)
            raise PyRestServiceClientError(f"Unexpected error: {str(e)}")

    async def get_py_rest(self, request: GetPyRestRequest) -> GetPyRestResponse:
        """Get a single py_rest by ID.
        
        Args:
            request: Request containing the py_rest ID
            
        Returns:
            Response containing the requested py_rest
            
        Raises:
            PyRestServiceClientError: If the API call fails
        """
        logger.info("Getting py_rest", py_rest_id=request.id)
        
        try:
            # Make authenticated REST API call
            response = await self._make_authenticated_request(
                "GET",
                f"{self.base_url}/api/v1/py_rests/{request.id}"
            )
            
            # Handle response
            if response.status_code == 200:
                response_data = response.json()
                result = GetPyRestResponse(**response_data)
                logger.info("PyRest retrieved successfully", py_rest_id=result.py_rest.id)
                return result
            else:
                await self._handle_error_response(response, f"getting py_rest {request.id}")
                
        except httpx.RequestError as e:
            logger.error("Network error getting py_rest", error=str(e), py_rest_id=request.id)
            raise PyRestServiceClientError(f"Network error: {str(e)}")
        except Exception as e:
            logger.error("Unexpected error getting py_rest", error=str(e), py_rest_id=request.id, exc_info=True)
            raise PyRestServiceClientError(f"Unexpected error: {str(e)}")

    async def update_py_rest(self, py_rest: PyRestDto) -> UpdatePyRestResponse:
        """Update an existing py_rest.
        
        Args:
            py_rest: Updated py_rest data
            
        Returns:
            Response containing the updated py_rest
            
        Raises:
            PyRestServiceClientError: If the API call fails
        """
        if not py_rest.id:
            raise PyRestServiceClientError("PyRest ID is required for update operations")
            
        logger.info("Updating py_rest", py_rest_id=py_rest.id)
        
        try:
            # Convert DTO to JSON payload (exclude ID from body, it's in the URL)
            payload = py_rest.model_dump(exclude_none=True, exclude={'id'})
            
            # Make authenticated REST API call
            response = await self._make_authenticated_request(
                "PUT",
                f"{self.base_url}/api/v1/py_rests/{py_rest.id}",
                json=payload
            )
            ...
            if response.status_code == 200:
                response_data = response.json()
                result = UpdatePyRestResponse(**response_data)
                logger.info("PyRest updated successfully", py_rest_id=result.py_rest.id)
                return result
            else:
                await self._handle_error_response(response, f"updating py_rest {py_rest.id}")
                
        except httpx.RequestError as e:
            logger.error("Network error updating py_rest", error=str(e), py_rest_id=py_rest.id)
            raise PyRestServiceClientError(f"Network error: {str(e)}")
        except Exception as e:
            logger.error("Unexpected error updating py_rest", error=str(e), py_rest_id=py_rest.id, exc_info=True)
            raise PyRestServiceClientError(f"Unexpected error: {str(e)}")

    async def delete_py_rest(self, request: DeletePyRestRequest) -> DeletePyRestResponse:
        """Delete a py_rest by ID.
        
        Args:
            request: Request containing the py_rest ID to delete
        ...
        Raises:
            PyRestServiceClientError: If the API call fails
        """
        logger.info("Deleting py_rest", py_rest_id=request.id)
        ...
        try:
            ...
            if response.status_code == 200:
                response_data = response.json()
                result = DeletePyRestResponse(**response_data)
                logger.info("PyRest deleted successfully", message=result.message)
                return result
            else:
                await self._handle_error_response(response, f"deleting py_rest {request.id}")
                
        except httpx.RequestError as e:
            logger.error("Network error deleting py_rest", error=str(e), py_rest_id=request.id)
            raise PyRestServiceClientError(f"Network error: {str(e)}")
        except Exception as e:
            logger.error("Unexpected error deleting py_rest", error=str(e), py_rest_id=request.id, exc_info=True)
            raise PyRestServiceClientError(f"Unexpected error: {str(e)}")
    ...
    async def __aenter__(self) -> "PyRestServiceClient":
    ...
    def __enter__(self) -> "PyRestServiceClient":
    ...
    async def _handle_error_response(self, response: httpx.Response, operation: str) -> None:
        ...
        Raises:
            PyRestServiceClientError: Always raises with appropriate error message
        """
        try:
        ...
        raise PyRestServiceClientError(
